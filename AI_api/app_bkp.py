...
def save_model(model, query, hours, model_name, scaler=None):
    """Salva il modello e lo scaler (se presente) in un file .pkl con nome strutturato e pulito."""
    sanitized_query = sanitize_filename(query)
    model_path = os.path.join(MODEL_DIR, f"{model_name}_{sanitized_query}_{hours}h.pkl")
    with open(model_path, "wb") as f:
        pickle.dump({"model": model, "scaler": scaler}, f)
    return model_path

def load_model(query, hours, model_name):
    """Carica un modello se esiste, altrimenti restituisce None."""
    sanitized_query = sanitize_filename(query)
    model_path = os.path.join(MODEL_DIR, f"{model_name}_{sanitized_query}_{hours}h.pkl")
...
def train_model(req: TrainRequest):
    ...
    if model_name in ["GRU", "LSTM"]:
        ...
        save_model(model, req.query, req.training_hours, model_name, scaler=scaler)
        ...
        # 🔹 Salva su disco
        save_model(model_fit, req.query, req.training_hours, model_name)
        ...
        # 🔹 Salva su disco
        save_model(model_fb, req.query, req.training_hours, model_name)
...
def compute_rmse_endpoint(req: ComputeRMSERequest):
    ...
    for model_name in req.models:
        ...
        model_info = load_model(req.query, req.training_hours, mdl)
...
def infer_model(req: InferRequest):
    ...
    # Carichiamo da file
    model_info = load_model(req.query, req.training_hours, model_name)
    if model_info is None:
